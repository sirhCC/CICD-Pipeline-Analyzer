...
const logger = new logger_1.Logger('EnhancedAnalyticsRoutes');
// Lazy initialization of services to avoid circular dependencies and database initialization issues
let enhancedStatisticalAnalyticsService;
let optimizationIntegrationService;
...
router.post('/anomalies', async (req, res) => {
    ...
    try {
        ...
        if (!data || !Array.isArray(data) || data.length < 10) {
            return res.status(400).json(createResponse(false, null, 'Invalid data: requires array with at least 10 data points'));
        }
        ...
        return res.status(500).json(createResponse(false, null, error instanceof Error ? error.message : 'Unknown error'));
    }
});
/**
...
router.post('/trends', async (req, res) => {
    ...
    try {
        ...
        if (!data || !Array.isArray(data) || data.length < 5) {
            return res.status(400).json(createResponse(false, null, 'Invalid data: requires array with at least 5 data points'));
        }
...
router.post('/benchmark', async (req, res) => {
    ...
    try {
        ...
        if (typeof currentValue !== 'number' || !historicalData || !Array.isArray(historicalData) || historicalData.length < 5) {
            return res.status(400).json(createResponse(false, null, 'Invalid data: requires currentValue (number) and historicalData (array with at least 5 points)'));
        }
        logger.info('Enhanced benchmark request', {
            currentValue,
        ...
        const result = await enhancedStatisticalAnalyticsService.generateBenchmark(currentValue, historicalData, category);
        const executionTime = Date.now() - startTime;
        const response = {
            benchmark: result,
            insights: {
 ...
 */
router.get('/optimization/profiles', async (req, res) => {
    try {
 ...
 */
router.post('/optimization/profiles/:profileName', async (req, res) => {
    try {
        ...
        if (success) {
            return res.json(createResponse(true, { profile: profileName }, `Switched to ${profileName} profile successfully`));
        }
        else {
            return res.status(404).json(createResponse(false, null, `Profile '${profileName}' does not exist`));
        }
    }
    catch (error) {
...
// Helper functions
function calculateSeverityDistribution(results) {
...
function classifyTrendStrength(correlation) {
...
function assessTrendReliability(rSquared) {
    if (rSquared >= 0.8)
...
function calculateImprovementPotential(result) {
    return Math.max(0, 100 - result.percentile);
}
function generatePerformanceRecommendations(result) {
