...
import type { ValidationSchema } from '../middleware/request-validation';
...
router.get(
  ...
  asyncHandler(
    async (req: TypedRequest<{}, { period?: string; pipelineId?: string }>, res: Response) => {
      ...
      res.json(
        ResponseBuilder.success(dashboard, {
...
router.get(
  ...
  asyncHandler(
    async (req: TypedRequest<{ pipelineId: string }, { period?: string }>, res: Response) => {
      const startTime = Date.now();
...
router.get(
  ...
  asyncHandler(async (req: TypedRequest<{ pipelineId: string }>, res: Response) => {
    ...
    res.json(
      ResponseBuilder.success(patterns, {
        performance: { executionTime: Date.now() - startTime },
      })
    );
  })
 ...
 */
router.get(
  '/pipelines/:pipelineId/recommendations',
  requirePermission(Permission.PIPELINES_READ),
  validateRequest(pipelineParamsValidation),
  asyncHandler(async (req: TypedRequest<{ pipelineId: string }>, res: Response) => {
    const startTime = Date.now();
    ...
    const recommendations =
      await getAnalyticsService().generateOptimizationRecommendations(pipelineId);

    res.json(
      ResponseBuilder.success(recommendations, {
        performance: { executionTime: Date.now() - startTime },
      })
    );
  })
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json(
      ResponseBuilder.success(alerts, {
        performance: { executionTime: Date.now() - startTime },
      })
    );
  })
...
router.put(
  ...
  asyncHandler(
    async (
      ...
      res.json(
        ResponseBuilder.success(updatedAlert, {
          performance: { executionTime: Date.now() - startTime },
        })
      );
    }
